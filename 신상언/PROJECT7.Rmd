...
```
# 신체가 포도당을 처리하는 능력 & 당뇨 상관 관계
```{r}
mycolor <- ifelse(df$Outcome == 0, 'red', 'green')
plot(df$Glucose, col = mycolor, pch = 19, cex = 1)
```
## 인슐린 & 글루코즈 상관 관계
```{r}
...
## 
```{r}
plot(df$Age, df$Insulin, col = mycolor, pch = 19, cex = 1)
```
##
```{r}
plot(df$SkinThickness, df$Insulin, col = mycolor, pch = 19, cex = 1)
```
...
## 비만도, 인슐린 상관 관계(3) 
```{r}
cor(df1$Insulin, df1$BMI)
cor(df0$Insulin, df0$BMI)
