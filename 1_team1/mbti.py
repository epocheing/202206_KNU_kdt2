...
def manu1_pushed(root):
    ...
    button_frame.place(
        x=0, y=140, width=WIDTH - MENU_WIDTH + BUTTON_WIDTH, height=BUTTON_HEIGHT
    )
    ...
    test_button = tk.Button(
        button_frame,
        text="test start",
        command=lambda: test_button_pushed(test, question_label, detail_text),
        repeatdelay=1000,
    )
    l_button = tk.Button(
        button_frame,
        text="Left",
        command=lambda: L_button_pushed(test, question_label, detail_text),
        repeatdelay=1000,
    )
    r_button = tk.Button(
        button_frame,
        text="Right",
        command=lambda: R_button_pushed(test, question_label, detail_text),
        repeatdelay=1000,
    )
    go_disc_button = tk.Button(
        button_frame,
        text="상세 설명으로",
        command=lambda: manu2_pushed(root, mbti_type=test.mbti_type),
        repeatdelay=1000,
    )
    ...
    r_button.place(
        x=20 + BUTTON_WIDTH * 2, y=0, height=BUTTON_HEIGHT, width=BUTTON_WIDTH
    )
    go_disc_button.place(
        x=20 + BUTTON_WIDTH * 3, y=0, height=BUTTON_HEIGHT, width=BUTTON_WIDTH
    )
    ...
    for i in range(len(mbti)):
        rad.append(
            tk.Radiobutton(
                ...
                variable=mbti_index,
                value=i,
                command=lambda: radio_choice(detail_text, mbti_index),
                bg=FRAME_COLOR,
            )
        )
    ...
    for i in range(len(mbti)):
        rad1.append(
            tk.Radiobutton(
                radio_frame,
                text=mbti[i],
                variable=mbti1_index,
                value=i,
                command=lambda: mate_choice(mate_text1, mbti1_index),
                bg=FRAME_COLOR,
            )
        )
    ...
    for i in range(len(mbti)):
        rad2.append(
            tk.Radiobutton(
                radio_frame,
                text=mbti[i],
                variable=mbti2_index,
                value=i,
                command=lambda: mate_choice(mate_text2, mbti2_index),
                bg=FRAME_COLOR,
            )
        )
...
def manu4_pushed(root):
    ...
    st_mate_button = tk.Button(
        frame,
        text="MBTI 궁합 계산",
        bg=BUTTON_COLOR,
        repeatdelay=1000,
        command=lambda: student_mate_pushed(
            heart_text, result_label, name_text1, name_text2, mbti_text1, mbti_text2
        ),
    )
    st_mate_button.place(x=450, y=220, height=BUTTON_HEIGHT, width=BUTTON_WIDTH)

    st_reset_button = tk.Button(
        frame,
        text="초기화",
        bg=BUTTON_COLOR,
        repeatdelay=1000,
    ...
    st_reset_button.place(
        x=450, y=220 + BUTTON_HEIGHT, height=BUTTON_HEIGHT, width=BUTTON_WIDTH
    )
...
# 반환값 : Nan
# --------------------------
def student_mate_pushed(
    heart_text, result_label, name_text1, name_text2, mate_text1, mate_text2
):
...
if __name__ == "__main__":
    ...
    # gui 그리기
    # 메뉴
    canvas = tk.Canvas(root, height=HEIGHT, width=MENU_WIDTH, bg="#fff")

    canvas.place(x=0, y=0, height=HEIGHT, width=MENU_WIDTH)
    canvas.create_rectangle(
        0, 0, MENU_WIDTH, HEIGHT, outline=MENU_COLOR, fill=MENU_COLOR
    )
